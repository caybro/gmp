...
//  timer.start();
  ...
  query.prepare(QStringLiteral("INSERT INTO Tracks (path, url, title, artist, album, "
                               "year, genre, trackNo, length) "
                               "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"));
  ...
  for (const auto &rootPath : qAsConst(m_rootPaths)) { // TODO watch paths for changes
    ...
    while (it.hasNext()) {
      ...
      titles.append(tag->title().toCString(true));
      artists.append(tag->artist().toCString(true));
      albums.append(album);
      years.append(tag->year());
      genres.append(genre);
      trackNos.append(tag->track() + pos);
...
QUrl DbIndexer::coverArtForAlbum(const QString &album) const
{
  ...
  QList<QUrl> tracks;
  tracks.reserve(q.size());
  while (q.next()) {
    tracks.append(q.value(0).toUrl());
  }
  q.finish();
                             ...
                             int year, const QString &genre)
{
  ...
  QSqlQuery query;
  query.prepare(QStringLiteral("UPDATE Tracks SET title=?, artist=?, album=?, year=?, "
                               "genre=? WHERE url=?"));
...
{
  qWarning() << "!! SAVE ALBUM METADATA" << album << artist << "; cover URL:" << albumCover;
  ...
  if (!q.exec()) {
    qWarning() << "Failed to get list of tracks for album:" << album << "and artist:" << artist
               << "; error:" << q.lastError();
    return false;
  }
  ...
  while (q.next()) {
    paths.append(q.value(1).toString());
  }
  q.finish();
